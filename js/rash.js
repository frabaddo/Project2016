...
jQuery.fn.extend({
    ...
    addHeaderHTML: function() {
        ...
        /* Header title */
        var header = $("<header class=\"page-header cgen noselect todelete \"></header>"); //aggiunto noselect per rendere non selezionabile (Sinagra)
        ...
        /* Header author */
        var list_of_authors = [];
        $("#RASHhead meta[name='dc.creator']").each(function() {
            var current_value = $(this).attr("name");
            var current_id = $(this).attr("about");
            var current_name = $(this).attr("content");
            var current_email = $("#RASHhead meta[about='" + current_id + "'][property='schema:email']").attr("content");
            var current_affiliations = [];
            $("#RASHhead link[about='" + current_id + "'][property='schema:affiliation']").each(function() {
                var cur_affiliation_id = $(this).attr("href");
                current_affiliations.push($("#RASHhead meta[about='" + cur_affiliation_id + "'][property='schema:name']").attr("content"));
            });
...
$(function() {
    ...
    $("section[role=doc-footnotes]").prepend("<h1 class='noselect todelete'>Footnotes</h1>");
    ...
    $(figurebox_selector).each(function() {
        ...
        cur_caption.html("<span class=\"todelete noselect\"><strong class=\"cgen\">Figure " + cur_number + ". </strong></span>" + cur_caption.html());
    });
    $(tablebox_selector).each(function() {
        var cur_caption = $(this).parents("figure").find("figcaption");
        var cur_number = $(this).findNumber(tablebox_selector);
        cur_caption.html("<span class=\"todelete noselect\"><strong class=\"cgen\">Table " + cur_number + ". </strong></span>" + cur_caption.html());
    });
    $(formulabox_selector).each(function() {
        ...
        cur_caption.html(cur_caption.html() + "<span class=\"cgen todelete noselect\"> (" + cur_number + ")</span>");
    });
    $(listingbox_selector).each(function() {
        var cur_caption = $(this).parents("figure").find("figcaption");
        var cur_number = $(this).findNumber(listingbox_selector);
        cur_caption.html("<span class=\"todelete noselect\"><strong class=\"cgen\">Listing " + cur_number + ". </strong></span>" + cur_caption.html());
    });
    /* /END Captions */
        ...
        if ($.trim($(this).text()) == '') {
           ...
           if (referenced_element.length > 0) { 
                ...
                /* Bibliographic references */
                    ...
                    $(this).addClass("cgen");
                    $(this).addClass("noselect");                                                                       //da verificare
                /* Footnote references */
